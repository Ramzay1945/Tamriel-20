...
###can't change civic start
		...
		if iLeader ==gc.getInfoTypeForString("LEADER_DWEMER"):
			if eCivic ==gc.getInfoTypeForString("CIVIC_THEOCRACY") or eCivic ==gc.getInfoTypeForString("CIVIC_DIVINE_RIGHT") or eCivic ==gc.getInfoTypeForString("CIVIC_ORGANIZED_RELIGION") or eCivic ==gc.getInfoTypeForString("CIVIC_PAGANISM"):
				return True
		elif iLeader ==gc.getInfoTypeForString("LEADER_VIVEC") or iLeader ==gc.getInfoTypeForString("LEADER_ALMALEXIA") or iLeader ==gc.getInfoTypeForString("LEADER_NELOT"):
			if eCivic ==gc.getInfoTypeForString("CIVIC_HOUSES_BAN"):
				return True

###can't change civic end        

# LPlate - new code for Ramzay1945
		iCivicOption = gc.getCivicInfo(eCivic).getCivicOptionType()
		iCivic = gc.getCivicInfo(eCivic).getCivicOptionType()
		StateBelief = pPlayer.getStateReligion()
		if iCivicOption == gc.getInfoTypeForString('CIVICOPTION_8_DIVINES'):
			if not (pPlayer.getStateReligion() == gc.getInfoTypeForString('RELIGION_EIGHT_DIVINES')) and not iCivic == gc.getInfoTypeForString('CIVIC_8_DIVINES_NONE'):
				return True
		elif iCivicOption == gc.getInfoTypeForString('CIVICOPTION_DAEDRA'):
			if not (pPlayer.getStateReligion() == gc.getInfoTypeForString('RELIGION_ISLAM')) and not iCivic == gc.getInfoTypeForString('CIVIC_DAEDRA_NONE'):
				return True
		elif iCivicOption == gc.getInfoTypeForString('CIVICOPTION_NORDIC'):
			if not (pPlayer.getStateReligion() == gc.getInfoTypeForString('RELIGION_CHRISTIANITY')) and not iCivic == gc.getInfoTypeForString('CIVIC_NORDIC_NONE'):
				return True
		elif iCivicOption == gc.getInfoTypeForString('CIVICOPTION_ALTMER'):
			if not (pPlayer.getStateReligion() == gc.getInfoTypeForString('RELIGION_JUDAISM')) and not iCivic == gc.getInfoTypeForString('CIVIC_ALTMER_NONE'):
				return True
		elif iCivicOption == gc.getInfoTypeForString('CIVICOPTION_TRIBUNAL'):
			if not (pPlayer.getStateReligion() == gc.getInfoTypeForString('RELIGION_TRIBUNAL')) and not iCivic == gc.getInfoTypeForString('CIVIC_TRIBUNAL_NONE'):
				return True
# End LPlate - new code for Ramzay1945
	...
	def citiesDestroyFeatures(self,argsList):
		iX, iY= argsList
##platyping code
##не совсем то, что нужно. Надо, что бы город можно было построить только в лесу, в данном случае лес не исчезает.
#
		pPlot = CyMap().plot(iX, iY)
		pPlayer = gc.getPlayer(pPlot.getOwner())
		if pPlayer.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_RUSSIA") and pPlot.getFeatureType() == gc.getInfoTypeForString("FEATURE_FOREST"):
			return False
##
		return True
