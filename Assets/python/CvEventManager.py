...
## mechaerik War Prize ModComp START##
		...
		iPromo1 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_I')
		iPromo2 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_II')
		iPromo3 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_III')
		if not (gc.getPlayer(pWinner.getOwner()).isBarbarian()):
			if (unitX.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_VAMPIRE")):
				if (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MELEE")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_ARCHER")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_RECON")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MAGE")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MOUNTED")):
					if CyGame().getSorenRandNum(100, "Bob") <= 33:
						iUnit = gc.getInfoTypeForString('UNIT_WILD_VAMPIRE')
						...
						#myResult = pCheckUnit.getUnitType()
						CyInterface().addMessage(CyGame().getActivePlayer(),True,5,"The new vampires converted.",'',0,'',ColorTypes(8),0,0,True,True)

#			if pWinner.getUnitType() == gc.getInfoTypeForString('UNIT_WILD_VAMPIRE'):
##			if (unitX.getUnitType() == gc.getInfoTypeForString('UNIT_WILD_VAMPIRE')):
#				iPromo1 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_I')
#				iPromo2 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_II')
#				iPromo3 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_III')
#				if pWinner.isHasPromotion(iPromo3):
#					iUnit = gc.getInfoTypeForString('UNIT_VAMPIRE')
#					eUnitAI = pWinner.getUnitAIType()
#					newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_VAMPIRE'), pWinner.getX(), pWinner.getY(), eUnitAI, DirectionTypes.DIRECTION_NORTH)
#					for i in range(gc.getNumPromotionInfos()):
#						if not i == iPromo1 and not i == iPromo2 and not i == iPromo3:
#							newUnit.setHasPromotion(i, pWinner.isHasPromotion(i))
#					newUnit.finishMoves()
#					pWinner.kill(True, PlayerTypes.NO_PLAYER)
#
			if (unitX.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_NECROMANCER")):
				if (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MELEE")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_ARCHER")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_RECON")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MAGE")) or (unitY.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MOUNTED")):
					if CyGame().getSorenRandNum(100, "Bob") <= 50:
						iSpawnRand = CyGame().getSorenRandNum(100, "Selecting Undead Unit")
						if iSpawnRand <= 60:
							iUnit = gc.getInfoTypeForString('UNIT_SKELETON')
							newUnit = pPlayer.initUnit(iUnit, pWinner.getX(), pWinner.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
							newUnit.finishMoves()
							newUnit.setDamage(50, pWinner.getOwner())
							CyInterface().addMessage(CyGame().getActivePlayer(),True,5,"The new skeleton rised.",'',0,'',ColorTypes(8),0,0,True,True)
						elif iSpawnRand <= 85:
							iUnit = gc.getInfoTypeForString('UNIT_SKELETON_LORD')
							newUnit = pPlayer.initUnit(iUnit, pWinner.getX(), pWinner.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
							newUnit.finishMoves()
							newUnit.setDamage(50, pWinner.getOwner())
							CyInterface().addMessage(CyGame().getActivePlayer(),True,5,"The new skeleton lord rised.",'',0,'',ColorTypes(8),0,0,True,True)
						elif iSpawnRand <= 95:
							iUnit = gc.getInfoTypeForString('UNIT_GHOST')
							newUnit = pPlayer.initUnit(iUnit, pWinner.getX(), pWinner.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
							newUnit.finishMoves()
							newUnit.setDamage(50, pWinner.getOwner())
							CyInterface().addMessage(CyGame().getActivePlayer(),True,5,"The new ghost rised.",'',0,'',ColorTypes(8),0,0,True,True)
						else:
							iUnit = gc.getInfoTypeForString('UNIT_LEECH')
							newUnit = pPlayer.initUnit(iUnit, pWinner.getX(), pWinner.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
							newUnit.finishMoves()
							newUnit.setDamage(50, pWinner.getOwner())
							CyInterface().addMessage(CyGame().getActivePlayer(),True,5,"The new lich rised.",'',0,'',ColorTypes(8),0,0,True,True)

## War Prize Modcomp END##
## LPlate2 Vampire code##
#		pAttacker = gc.getPlayer(pWinner.getOwner())
#		if pWinner.getUnitType() == gc.getInfoTypeForString('UNIT_WILD_VAMPIRE'):
#			iPromo1 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_I')
#			iPromo2 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_II')
#			iPromo3 = gc.getInfoTypeForString('PROMOTION_VAMPIRE_III')
...
#				iUnit = gc.getInfoTypeForString('UNIT_VAMPIRE')
#				eUnitAI = pWinner.getUnitAIType()
#				newUnit = pAttacker.initUnit(iUnit, pWinner.getX(), pWinner.getY(), eUnitAI, DirectionTypes.DIRECTION_NORTH)
#				for i in range(gc.getNumPromotionInfos()):
#					if not i == iPromo1 and not i == iPromo2 and not i == iPromo3:
#						newUnit.setHasPromotion(i, pWinner.isHasPromotion(i))
#				newUnit.finishMoves()
#				pWinner.kill(True, PlayerTypes.NO_PLAYER)
#			elif pWinner.isHasPromotion(iPromo2):
#				pWinner.setHasPromotion(iPromo3, True)
#			elif pWinner.isHasPromotion(iPromo1):
#				pWinner.setHasPromotion(iPromo2, True)
#			else:
#				pWinner.setHasPromotion(iPromo1, True)
## LPlate2 Vampire code END##
...
# Code for Dwemer no religion
		iCiv = pPlayer.getCivilizationType()
		lSpecCitiesCivs = ['CIVILIZATION_DWEMER'] # Ramzay, add in the civs you want here (keep aligned with showVictoryConditionScreen in CvVictoryScreen.py)

# LPlate - new code for Ramzay1945
		if iOldReligion == gc.getInfoTypeForString('RELIGION_EIGHT_DIVINES'):
			pPlayer.setCivics(gc.getInfoTypeForString('CIVICOPTION_8_DIVINES'), gc.getInfoTypeForString('CIVIC_8_DIVINES_NONE'))
		elif iOldReligion == gc.getInfoTypeForString('RELIGION_ISLAM'):
			pPlayer.setCivics(gc.getInfoTypeForString('CIVICOPTION_DAEDRA'), gc.getInfoTypeForString('CIVIC_DAEDRA_NONE'))
		elif iOldReligion == gc.getInfoTypeForString('RELIGION_CHRISTIANITY'):
			pPlayer.setCivics(gc.getInfoTypeForString('CIVICOPTION_NORDIC'), gc.getInfoTypeForString('CIVIC_NORDIC_NONE'))
		elif iOldReligion == gc.getInfoTypeForString('RELIGION_JUDAISM'):
			pPlayer.setCivics(gc.getInfoTypeForString('CIVICOPTION_ALTMER'), gc.getInfoTypeForString('CIVIC_ALTMER_NONE'))
		elif iOldReligion == gc.getInfoTypeForString('RELIGION_TRIBUNAL'):
			pPlayer.setCivics(gc.getInfoTypeForString('CIVICOPTION_TRIBUNAL'), gc.getInfoTypeForString('CIVIC_TRIBUNAL_NONE'))
# End LPlate - new code for Ramzay1945
# Code for Dwemer no religion
		if (eTeam.isAlive() and not eTeam.isMinorCiv() and not eTeam.isBarbarian()):
			for iSpecCiv in lSpecCitiesCivs:
				if pPlayer.getCivilizationType() == gc.getInfoTypeForString(iSpecCiv):
					pPlayer.setCivics(gc.getInfoTypeForString('CIVICOPTION_RELIGION'), gc.getInfoTypeForString('CIVIC_DEFIANCE'))
...
#		return CyGame().getSorenRandNum(iNum, "GenghisKhanScenario")
#
##################################################################
