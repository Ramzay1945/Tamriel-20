...
class CvMainInterface:
	...
	def updatePlotListButtons( self ):
		...
		## Ethnic Buildings (Asaf) - Start ##
		if pHeadSelectedCity != None:
			CyGame().setBuildingModelCity(-1, -1)
		## Ethnic Buildings (Asaf) - End ##
				...
				for i in range ( g_NumBuildingClassInfos ):
					if (not isLimitedWonderClass(i)):
						...
						if (pHeadSelectedCity.canConstruct(eLoopBuilding, False, True, False)):
							## Ethnic Buildings (Asaf) - Start ##						
							szButton = gc.getPlayer(pHeadSelectedCity.getOwner()).getBuildingButton(eLoopBuilding)
														
							screen.appendMultiListButton( "BottomButtonContainer", szButton, iRow, WidgetTypes.WIDGET_CONSTRUCT, i, -1, False )
...
#####################################################	
	def updateGreatGeneralBar(self, screen):
		if not CyInterface().isCityScreenUp():
			if CyUserProfile().getGraphicOption(gc.getInfoTypeForString("GRAPHICOPTION_GREAT_GENERAL_BAR")):
				...
				xResolution = screen.getXResolution()
				if (xResolution >= 1280):
					screen.setImageButton( "GreatGeneralBarIcon", ArtFileMgr.getUnitArtInfo("ART_DEF_UNIT_WARLORD_ANCIENT").getButton(), 500, yCoord - 3, 24, 24, WidgetTypes.WIDGET_HELP_GREAT_GENERAL, -1, -1 )
					screen.show( "GreatGeneralBarIcon" )
					screen.setLabel( "GreatGeneralBarText", "Background", szText, CvUtil.FONT_CENTER_JUSTIFY, 575, yCoord, -0.4, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_GREAT_GENERAL, -1, -1 )
