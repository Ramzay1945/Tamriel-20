...
## Unique Civics ##
	def placeUnit(self):
		...
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_SPECIAL_ABILITIES", ()), "", false, true,
		...
		for i in UniqueCivics.UniqueCivics().Civics:
			if self.iCivilization != gc.getInfoTypeForString(i[0]): continue
			iUnique = gc.getInfoTypeForString(i[2])
			if iUnique == -1: continue
			screen.attachImageButton(panelName, "", gc.getCivicInfo(iUnique).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIVIC, iUnique, 1, False )
## Unique Civics ##	
