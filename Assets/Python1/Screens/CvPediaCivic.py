...
## Unique Civics ##
	...
	def placeCiv(self):
		...
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_CATEGORY_CIV", ()), "", false, true,
				 self.X_REQUIRES, self.Y_MAIN_PANE - 20, self.W_REQUIRES, self.H_REQUIRES, PanelStyles.PANEL_STYLE_BLUE50 )
		screen.enableSelect(panelName, False)
		
		for i in UniqueCivics.UniqueCivics().Civics:
			iCiv = gc.getInfoTypeForString(i[0])
			if iCiv == -1: continue
			if self.iCivic == gc.getInfoTypeForString(i[2]):
...
## Unique Civics ##
		for i in UniqueCivics.UniqueCivics().Civics:
			...
			iSpecificCivic = gc.getInfoTypeForString(i[2])
			if iSpecificCivic == -1: continue
			if self.iCivic == iSpecificCivic:
				szSpecialText += "\n" + CyTranslator().getText("TXT_KEY_REPLACES_UNIT", (gc.getCivicInfo(iGenericCivic).getDescription(),))
			if self.iCivic == iGenericCivic:
				szSpecialText += "\n" + CyTranslator().getText("TXT_KEY_REPLACED_BY_UNIT", (gc.getCivicInfo(iSpecificCivic).getDescription(),))
## Unique Civics ##
